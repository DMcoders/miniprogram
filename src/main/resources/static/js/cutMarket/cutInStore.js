...
$(document).ready(function () {
    $("#cutStoreQcode")[0].focus();

    document.onkeyup = function (event) {
        e = event ? event : (window.event ? window.event : null);
        if (e.keyCode==13 && e.target.getAttribute("id")=="cutStoreQcode") {
            $("#tailorQcode").focus();
...
function inStore() {
    ...
    if(tailorQcode.length>1) {
        swal({type:"warning",title:"",text: "<span style=\"font-weight:bolder;font-size: 20px\">只需扫描一扎！</span>",html: true});
        return false;
    }
    var inStoreJson = {};
    inStoreJson.cutStoreLocation = $("#cutStoreQcode").val();
    inStoreJson.tailorQcode = tailorQcode;

    $.ajax({
        url: basePath + "inStore",
        type:'POST',
        data: {inStoreJson:JSON.stringify(inStoreJson)},
        success: function (data) {
            if(data) {
                ...
                swal({
                        type:"success",
                        title:"",
                        text: "<span style=\"font-weight:bolder;color:#F00;font-size: 50px\">共"+ data+ "扎,入库成功！</span>",
                        html: true
                    },
                    function(){
